...
final class ArrayCacheIntClean {
    ...
    ArrayCacheIntClean(int bucketCapacity) {
    ...
    static void fill(int[] array, int fromIndex, int toIndex, int value) {
        ...
        if (MarlinConst.DO_CHECKS) {
            ArrayCacheIntClean.check(array, fromIndex, toIndex, value);
        }
    }
    ...
    static final class Bucket {
        ...
        void putArray(int[] array) {
            if (MarlinConst.DO_CHECKS && array.length != this.arraySize) {
                MarlinUtils.logInfo("ArrayCacheInt(Clean): bad length = " + array.length);
                return;
            }
            if (MarlinConst.DO_STATS) {
                ++this.stats.returnOp;
    ...
    static final class Reference {
        ...
        Reference(ArrayCacheIntClean cache, int initialSize) {
            this.cache = cache;
            this.initial = ArrayCacheIntClean.createArray(initialSize);
            if (MarlinConst.DO_STATS) {
        ...
        int[] getArray(int length) {
            ...
            if (MarlinConst.DO_LOG_OVERSIZE) {
                MarlinUtils.logInfo("ArrayCacheInt(Clean): getArray[oversize]: length=\t" + length);
            }
            return ArrayCacheIntClean.createArray(length);
        }
        ...
        int[] widenArray(int[] array, int usedSize, int needSize) {
            ...
            if (MarlinConst.DO_LOG_WIDEN_ARRAY) {
                MarlinUtils.logInfo("ArrayCacheInt(Clean): widenArray[" + res.length + "]: usedSize=\t" + usedSize + "\tlength=\t" + length + "\tneeded length=\t" + needSize);
            }
            return res;
        }

        boolean doSetRef(int[] array) {
            return array != this.initial;
        }

        int[] putArrayClean(int[] array) {
        ...
        int[] putArray(int[] array, int fromIndex, int toIndex) {
            if (array.length <= ArrayCacheConst.MAX_ARRAY_SIZE) {
                if (toIndex != 0) {
                    ArrayCacheIntClean.fill(array, fromIndex, toIndex, 0);
                }
