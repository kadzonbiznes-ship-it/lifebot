...
extends BaseLinkedAtomicQueue<E> {
    ...
    public boolean offer(E e) {
        ...
        LinkedQueueAtomicNode oldNode = this.lpProducerNode();
    ...
    public int fill(MessagePassingQueue.Supplier<E> s, int limit) {
        LinkedQueueAtomicNode<Object> tail;
        if (null == s) {
        ...
        LinkedQueueAtomicNode<Object> head = tail = this.newNode(s.get());
        for (int i = 1; i < limit; ++i) {
            LinkedQueueAtomicNode<Object> temp = this.newNode(s.get());
            tail.spNext(temp);
            tail = temp;
        }
        LinkedQueueAtomicNode oldPNode = this.lpProducerNode();
