...
class FactoryFinder {
    ...
    static <T> T newInstance(Class<T> type, String className, ClassLoader cl, boolean doFallback, boolean useBSClsLoader) throws FactoryConfigurationError {
        ...
        try {
            ...
            throw new FactoryConfigurationError(x, "Provider " + className + " could not be instantiated: " + String.valueOf(x));
        }
    }
    ...
    static <T> T find(Class<T> type, String fallbackClassName) throws FactoryConfigurationError {
        ...
        if ((factoryClassName = SecuritySupport.readConfig(factoryId)) != null) {
            return FactoryFinder.newInstance(type, factoryClassName, null, true);
        }
        T provider = FactoryFinder.findServiceProvider(type);
        if (provider != null) {
            return provider;
        }
