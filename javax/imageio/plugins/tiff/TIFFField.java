...
implements Cloneable {
    ...
    private static final boolean[] IS_INTEGRAL = new boolean[]{false, true, false, true, true, false, true, true, true, true, false, false, false, false};
    private TIFFTag tag;
    ...
    public static TIFFField createFromMetadataNode(TIFFTagSet tagSet, Node node) {
        ...
        if (tag == null) {
            tag = new TIFFTag("UnknownTag", tagNumber, 1 << type);
        }
        try {
    ...
    public TIFFField(TIFFTag tag, int type, int count, Object data) {
        ...
        if (!tag.isDataTypeOK(type)) {
            throw new IllegalArgumentException("Illegal data type " + type + " for " + tag.getName() + " tag");
        }
        if (count < 0) {
            throw new IllegalArgumentException("count < 0!");
        }
        if ((type == 5 || type == 10) && count < 1) {
            throw new IllegalArgumentException("Type is TIFF_RATIONAL or TIFF_SRATIONAL and count < 1");
        }
        if (type == 13 && count != 1) {
        ...
        block0 : switch (type) {
            ...
            case 6: 
            case 7: {
                isDataArrayCorrect = data instanceof byte[] && ((byte[])data).length == count;
                break;
            }
            case 2: {
                isDataArrayCorrect = data instanceof String[] && ((String[])data).length == count;
                break;
            }
            case 3: {
                isDataArrayCorrect = data instanceof char[] && ((char[])data).length == count;
                break;
            }
            case 4: {
                boolean bl = isDataArrayCorrect = data instanceof long[] && ((long[])data).length == count;
                if (!isDataArrayCorrect) break;
                for (long datum : (long[])data) {
                    if (datum < 0L) {
                        throw new IllegalArgumentException("Negative value supplied for TIFF_LONG");
                    }
                    if (datum <= 0xFFFFFFFFL) continue;
                    throw new IllegalArgumentException("Too large value supplied for TIFF_LONG");
                }
                break;
            }
            case 13: {
                boolean bl = isDataArrayCorrect = data instanceof long[] && ((long[])data).length == 1;
                if (((long[])data)[0] < 0L) {
                    throw new IllegalArgumentException("Negative value supplied for TIFF_IFD_POINTER");
                }
                if (((long[])data)[0] <= 0xFFFFFFFFL) break;
                throw new IllegalArgumentException("Too large value supplied for TIFF_IFD_POINTER");
            }
            case 5: {
                boolean bl = isDataArrayCorrect = data instanceof long[][] && ((long[][])data).length == count;
                if (!isDataArrayCorrect) break;
                for (long[] datum : (long[][])data) {
                    ...
                    if (datum[0] < 0L || datum[1] < 0L) {
                        throw new IllegalArgumentException("Negative value supplied for TIFF_RATIONAL");
                    }
                    if (datum[0] <= 0xFFFFFFFFL && datum[1] <= 0xFFFFFFFFL) continue;
                    throw new IllegalArgumentException("Too large value supplied for TIFF_RATIONAL");
                }
                break;
            }
            case 8: {
                isDataArrayCorrect = data instanceof short[] && ((short[])data).length == count;
                break;
            }
            case 9: {
                isDataArrayCorrect = data instanceof int[] && ((int[])data).length == count;
                break;
            }
            case 10: {
                boolean bl = isDataArrayCorrect = data instanceof int[][] && ((int[][])data).length == count;
                if (!isDataArrayCorrect) break;
                for (int[] datum : (int[][])data) {
                    if (datum.length == 2) continue;
                    isDataArrayCorrect = false;
                    break block0;
                }
                break;
            ...
            case 11: {
                isDataArrayCorrect = data instanceof float[] && ((float[])data).length == count;
                break;
            }
            case 12: {
                isDataArrayCorrect = data instanceof double[] && ((double[])data).length == count;
                break;
            }
            default: {
                throw new IllegalArgumentException("Unknown data type " + type);
    ...
    public TIFFField(TIFFTag tag, long value) {
        ...
        this.count = 1;
        if (value < 65536L) {
            if (!tag.isDataTypeOK(3)) {
                throw new IllegalArgumentException("Illegal data type " + TIFFField.getTypeName(3) + " for tag \"" + tag.getName() + "\"");
            }
            this.type = 3;
            char[] cdata = new char[]{(char)value};
            this.data = cdata;
        } else {
            if (!tag.isDataTypeOK(4)) {
                throw new IllegalArgumentException("Illegal data type " + TIFFField.getTypeName(4) + " for tag \"" + tag.getName() + "\"");
            }
            this.type = 4;
    ...
    public TIFFField(TIFFTag tag, int type, long offset, TIFFDirectory dir) {
        ...
        if (type != 4 && type != 13) {
            throw new IllegalArgumentException("type " + type + " is neither TIFFTag.TIFF_LONG nor TIFFTag.TIFF_IFD_POINTER");
        }
        if (offset <= 0L) {
    ...
    public static Object createArrayForType(int dataType, int count) {
        ...
        if ((dataType == 5 || dataType == 10) && count < 1) {
            throw new IllegalArgumentException("Type is TIFF_RATIONAL or TIFF_SRATIONAL and count < 1");
        }
        if (dataType == 13 && count != 1) {
            throw new IllegalArgumentException("Type is TIFF_IFD_POINTER and count != 1");
        }
        switch (dataType) {
            case 1: 
    ...
    public long getAsLong(int index) {
        switch (this.type) {
            ...
            case 11: {
                return (long)((float[])this.data)[index];
            }
            case 12: {
                return (long)((double[])this.data)[index];
            }
            case 10: {
                int[] ivalue = this.getAsSRational(index);
    ...
    public String getValueAsString(int index) {
        switch (this.type) {
            ...
            case 10: {
                int[] ivalue = this.getAsSRational(index);
                String srationalString = ivalue[1] != 0 && ivalue[0] % ivalue[1] == 0 ? ivalue[0] / ivalue[1] + "/1" : ivalue[0] + "/" + ivalue[1];
                return srationalString;
            }
            case 5: {
                long[] lvalue = this.getAsRational(index);
                String rationalString = lvalue[1] != 0L && lvalue[0] % lvalue[1] == 0L ? lvalue[0] / lvalue[1] + "/1" : lvalue[0] + "/" + lvalue[1];
    ...
    public TIFFField clone() throws CloneNotSupportedException {
        ...
        Object fieldData = switch (this.type) {
            case 1, 6, 7 -> ((byte[])this.data).clone();
            case 3 -> ((char[])this.data).clone();
            case 8 -> ((short[])this.data).clone();
            case 9 -> ((int[])this.data).clone();
            case 4, 13 -> ((long[])this.data).clone();
            case 11 -> ((float[])this.data).clone();
            case 12 -> ((double[])this.data).clone();
            case 10 -> ((int[][])this.data).clone();
            case 5 -> ((long[][])this.data).clone();
            case 2 -> ((String[])this.data).clone();
        ...
        field.dir = this.dir != null ? this.dir.clone() : null;
        return field;
    }
}
